...
class FilmApp extends HTMLElement{
  constructor(){super();this.history=[];this.active=null;this.picker='';this.selected=null;this.ratings={};this.searchTimer=null;this.saving=false;this.syncing=false;}
  ...
  enqueue(job){const q=this.queue();q.push({id:crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`,...job});this.saveQueue(q);queueMicrotask(()=>this.syncQueue());}
  ...
  async load(){try{const [h,c]=await Promise.all([api('getHistory',{limit:10000}),api('getCurrent')]);if(!h?.ok)throw Error(h?.error||'Historiken kunde inte hämtas');const pending=this.queue().filter(j=>j.type==='finish').map(j=>j.row);const remote=h.rows||[];const ids=new Set(remote.map(r=>`${r.Datum}|${r.Film}|${r['Vem valde']}`));this.history=[...pending.filter(r=>!ids.has(`${r.Datum}|${r.Film}|${r['Vem valde']}`)),...remote];localStorage.setItem('filmkvall_history_cache',JSON.stringify(this.history.slice(0,500)));if(!this.active&&c?.active)this.active={who:c.next,film:c.suggestion};this.dashboard();this.syncBadge();}catch{this.syncBadge();}}

  async syncQueue(){if(this.syncing||!navigator.onLine)return;if(!this.queue().length){this.syncBadge();return;}this.syncing=true;this.syncBadge();try{while(this.queue().length){const job=this.queue()[0];if(job.type==='start'){let cur=await api('getCurrent');for(let i=0;i<PEOPLE.length&&cur?.next!==job.who;i++){await api('skipNext');cur=await api('getCurrent');}const res=await api('startNight',{film:job.film,by:job.who});if(!res?.ok)throw Error(res?.error||'Kunde inte synka filmvalet');}else if(job.type==='finish'){const marker=`sync:${job.id}`;const recent=await api('getHistory',{limit:50});const already=(recent?.rows||[]).some(r=>String(r.Kommentar||'').includes(marker));if(!already){const s=await api('saveScores',{scores:JSON.stringify(job.scores)});if(!s?.ok)throw Error(s?.error||'Kunde inte synka betygen');const res=await api('saveNight',{who:job.who,film:job.film,comment:`${job.comment||''} · ${marker}`});if(!res?.ok)throw Error(res?.error||'Kunde inte synka kvällen');}}this.saveQueue(this.queue().filter(x=>x.id!==job.id));}this.status('Allt är synkat.');}catch{this.status('Sparat på iPad · synkar automatiskt senare.');}finally{this.syncing=false;this.syncBadge();}}
  ...
  start(){if(this.saving)return;this.saving=true;const m=this.selected;this.active={who:this.picker,film:movieTitle(m),mediaType:typeOf(m),externalId:String(m.id),poster:poster(m.poster_path),runtime:m.runtime||'',year:yearOf(m)};localStorage.setItem('filmkvall_active_meta',JSON.stringify(this.active));this.enqueue({type:'start',who:this.picker,film:this.active.film});this.dashboard();this.status('Sparat på iPad · synkas i bakgrunden.');this.saving=false;}
  ...
  finish(){if(this.saving)return;this.saving=true;const a=this.active;const legacy={};PEOPLE.forEach(p=>legacy[p]=this.ratings[p]==='up'?'10':this.ratings[p]==='neutral'?'5':this.ratings[p]==='down'?'1':'');const row={'Datum':new Date().toISOString().slice(0,10),'Film':a.film,'Vem valde':a.who};PEOPLE.forEach(p=>row[p]=legacy[p]);this.history.unshift(row);localStorage.setItem('filmkvall_history_cache',JSON.stringify(this.history.slice(0,500)));localStorage.removeItem('filmkvall_active_meta');this.active=null;this.enqueue({type:'finish',who:a.who,film:a.film,scores:legacy,comment:`${a.mediaType==='tv'?'Serie':'Film'}${a.year?` · ${a.year}`:''}`,row});this.dashboard();this.status('Klart! Sparat på iPad · databasen synkas i bakgrunden.');this.saving=false;}
}
