...
class FilmApp extends HTMLElement{
  ...
  async start(){const m=this.selected;this.status('Sparar kvällens val…');try{const res=await api('startNightFor',{who:this.picker,film:movieTitle(m)});if(!res?.ok)throw Error(res?.error||'Kunde inte spara');this.active={who:this.picker,film:movieTitle(m),mediaType:typeOf(m),externalId:String(m.id),poster:poster(m.poster_path),runtime:m.runtime||'',year:yearOf(m)};localStorage.setItem('filmkvall_active_meta',JSON.stringify(this.active));this.status('Valet är sparat. Ha en bra filmkväll!');this.dashboard();}catch(e){this.status(String(e.message||e),true);}}
  ...
  async finish(){this.status('Sparar betyg och uppdaterar statistiken…');try{const a=this.active;const res=await api('finishNight',{who:a.who,film:a.film,ratings:JSON.stringify(this.ratings),mediaType:a.mediaType||'movie',externalId:a.externalId||'',poster:a.poster||'',runtime:a.runtime||'',year:a.year||''});if(!res?.ok)throw Error(res?.error||'Kunde inte spara');localStorage.removeItem('filmkvall_active_meta');this.active=null;await this.load();this.status('Kvällen är sparad. Statistiken är uppdaterad.');}catch(e){this.status(String(e.message||e),true);}}
}
