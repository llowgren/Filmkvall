...
class FilmApp extends HTMLElement{
  constructor(){super();this.history=[];this.active=null;this.picker='';this.selected=null;this.ratings={};this.searchTimer=null;this.saving=false;}
  ...
  async start(){if(this.saving)return;this.saving=true;const m=this.selected;const btn=this.querySelector('[data-start]');if(btn){btn.disabled=true;btn.textContent='Sparar…';}this.status('Sparar kvällens val…');try{let cur=await api('getCurrent');for(let i=0;i<PEOPLE.length&&cur?.next!==this.picker;i++){await api('skipNext');cur=await api('getCurrent');}const res=await api('startNight',{film:movieTitle(m),by:this.picker});if(!res?.ok)throw Error(res?.error||'Kunde inte spara');this.active={who:this.picker,film:movieTitle(m),mediaType:typeOf(m),externalId:String(m.id),poster:poster(m.poster_path),runtime:m.runtime||'',year:yearOf(m)};localStorage.setItem('filmkvall_active_meta',JSON.stringify(this.active));this.status('Valet är sparat. Ha en bra filmkväll!');this.dashboard();}catch(e){this.status(String(e.message||e),true);if(btn){btn.disabled=false;btn.textContent=`Det här blir ${this.picker}s val`;}}finally{this.saving=false;}}
  ...
  async finish(){if(this.saving)return;this.saving=true;const btn=this.querySelector('[data-save]');if(btn){btn.disabled=true;btn.textContent='Sparar…';}this.status('Sparar betyg…');try{const a=this.active;const legacy={};PEOPLE.forEach(p=>legacy[p]=this.ratings[p]==='up'?'10':this.ratings[p]==='neutral'?'5':this.ratings[p]==='down'?'1':'');const scores=await api('saveScores',{scores:JSON.stringify(legacy)});if(!scores?.ok)throw Error(scores?.error||'Kunde inte spara betygen');const res=await api('saveNight',{who:a.who,film:a.film,comment:`${a.mediaType==='tv'?'Serie':'Film'}${a.year?` · ${a.year}`:''}`});if(!res?.ok)throw Error(res?.error||'Kunde inte spara kvällen');const row={'Datum':new Date().toISOString().slice(0,10),'Film':a.film,'Vem valde':a.who};PEOPLE.forEach(p=>row[p]=legacy[p]);this.history.unshift(row);localStorage.removeItem('filmkvall_active_meta');this.active=null;this.dashboard();this.status('Kvällen är sparad. Statistiken är uppdaterad.');}catch(e){this.status(String(e.message||e),true);if(btn){btn.disabled=false;btn.textContent='Försök spara igen';}}finally{this.saving=false;}}
}
