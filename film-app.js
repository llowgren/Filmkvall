...
function pickerLeaderboard(history){
  return PEOPLE.map(person=>{
    const picks=history.filter(r=>pickerOf(r)===person);
    const scores=picks.flatMap(r=>PEOPLE.filter(p=>p!==person).map(p=>ratingPoints(val(r,p))).filter(x=>x!==null));
    return {person,avg:scores.length?scores.reduce((a,b)=>a+b,0)/scores.length:null,votes:scores.length};
  }).filter(x=>x.votes).sort((a,b)=>b.avg-a.avg||b.votes-a.votes).slice(0,5);
}
...
async function recommendations(history, person, preferences=[]){
  const seen = new Set(history.map(r=>normalize(titleOf(r))));
  const mine = preferences.filter(p=>p.person===person);
  const blocked = new Set(mine.map(p=>`${p.type}:${p.id}`));
  const dislikedGenres = new Set(mine.filter(p=>p.kind==='dislike').flatMap(p=>p.genreIds||[]));
  ...
  if(!seeds.length){
    ...
    return (j?.results||[])
      .filter(x=>!blocked.has(`movie:${x.id}`))
      .map(x=>({...x,media_type:'movie',why:'Populär just nu',fit:(x.vote_average||0)-(x.genre_ids||[]).filter(id=>dislikedGenres.has(id)).length*1.25}))
  ...
  batches.flat().forEach(m=>{
    const k=`${typeOf(m)}:${m.id}`;
    if(seen.has(normalize(movieTitle(m)))||blocked.has(k)) return;
    const c=map.get(k)||{...m,hits:0,ownHits:0,seeds:[]}; c.hits++; c.ownHits+=m._own?1:0; c.seeds.push(m._seed); map.set(k,c);
  });
  return [...map.values()].map(m=>{
    const dislikePenalty=(m.genre_ids||[]).filter(id=>dislikedGenres.has(id)).length*1.25;
    return {...m,fit:m.ownHits*4+m.hits*2+(m.vote_average||0)-dislikePenalty,why:m.ownHits?`Liknar ${m.seeds[0]}`:`Kan passa flera i familjen`};
...
class FilmApp extends HTMLElement{
  ...
  preferences(){try{return JSON.parse(localStorage.getItem('filmkvall_preferences_v1')||'[]');}catch{return[];}}
  personPreferences(person=this.picker){return this.preferences().filter(p=>p.person===person);}
  markPreference(kind){
    const m=this.selected;if(!m||!this.picker)return;
    const type=typeOf(m),id=String(m.id),key=`${this.picker}|${type}:${id}`;
    const item={key,person:this.picker,kind,type,id,title:movieTitle(m),year:yearOf(m),genreIds:(m.genres||[]).map(g=>g.id).filter(Boolean),savedAt:Date.now()};
    const prefs=[item,...this.preferences().filter(p=>p.key!==key)].slice(0,500);
    localStorage.setItem('filmkvall_preferences_v1',JSON.stringify(prefs));
    const label=kind==='seen'?'Sett':'Ogillar';
    this.choose(this.picker);
    this.status(`${movieTitle(m)} markerad som ${label.toLowerCase()}.`);
  }
